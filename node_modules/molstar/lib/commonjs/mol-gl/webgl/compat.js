...
function getDrawBuffersIndexed(gl) {
    var ext = gl.getExtension('OES_draw_buffers_indexed');
    if (ext === null)
        return null;
    return {
...
function getDisjointTimerQuery(gl) {
    if (isWebGL2(gl)) {
        ...
        var ext = gl.getExtension('EXT_disjoint_timer_query_webgl2') || gl.getExtension('EXT_disjoint_timer_query');
        if (ext === null)
            return null;
        return {
...
function getProvokingVertex(gl) {
    if (isWebGL2(gl)) {
        var ext = gl.getExtension('WEBGL_provoking_vertex');
        if (ext) {
            return {
...
function getNoNonInstancedActiveAttribs(gl) {
    ...
    if (typeof navigator !== 'undefined') {
        var ffMatch = window.navigator.userAgent.match(/Firefox\/([0-9]+)\./);
        if (!ffMatch)
            return true;
        var ffVersion = parseInt(ffMatch[1]);
...
function testColorBuffer(gl, type) {
    // setup shaders
    var vertShader = (0, shader_1.getShader)(gl, { type: 'vert', source: TextureTestVertShader });
    var fragShader = (0, shader_1.getShader)(gl, { type: 'frag', source: TextureTestFragShader });
    if (!vertShader || !fragShader)
        return false;
    // setup program
    var program = (0, program_1.getProgram)(gl);
    ...
    if (type === gl.FLOAT) {
        ...
        if (error) {
            if (debug_1.isDebugMode) {
                console.log("error reading float pixels: '".concat((0, context_1.getErrorDescription)(gl, error), "'"));
            }
            return false;
        }
    }
