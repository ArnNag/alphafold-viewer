...
<?php 
		// Querying for sequence hits and putting the relevant hit information in JSON
...
";
	...
	// Querying for structure hits and putting the relevant hit information in JSON
	$structure_query = "SELECT astral_domain.sid, scop_node.sunid, model_start, model_end, domain_start_ATOM, domain_end_ATOM, z_score, translate_x, translate_y, translate_z, rotate_1_1, rotate_1_2, rotate_1_3, rotate_2_1, rotate_2_2, rotate_2_3, rotate_3_1, rotate_3_2, rotate_3_3
...
ORDER BY z_score DESC LIMIT 50;";
	...
	for ($x = 0; $x < $structure_n; $x++) {
		...
		$struct_domain_res[] = [intval($row["domain_start_ATOM"]), intval($row["domain_end_ATOM"])];
	...
	<script type="text/javascript">
	    ...
	    function superposeDomain(idx) {
		    ...
		    viewer.clear()
			    ...
			    .then(() => viewer.makeHiddenComponent(0, 20, 50)) // domain
			    .then(() => viewer.makeHiddenComponent(1, structModelRes[idx][0], structModelRes[idx][1])) // model
			    /* .then(() => viewer.makeHiddenComponent(1, 0, 177)) // model */
